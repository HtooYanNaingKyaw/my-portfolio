// Portfolio Configuration
// Set to true to show sections, false to hide them

export const portfolioConfig = {
  // Show/Hide Blog Section
  showBlog: false, // Set to true when you want to show blog posts
  
  // Show/Hide Experience Section
  showExperience: true, // Set to false if you don't want to show work experience
  
  // Show/Hide Education Section
  showEducation: true, // Set to false if you don't want to show education
  
  // Show/Hide Other Sections (for future use)
  showCertifications: true,
  
  // Contact Form Configuration
  enableContactForm: true,
  
  // Dark Mode Configuration
  enableDarkMode: true,
  
  // Animation Configuration
  enableAnimations: true,
};
